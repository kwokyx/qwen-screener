...
**能跑通的部分** ✅：
- 自然语言 Agent 先用本地确定性解析处理 PE/PB、ROE、股息率、净利润同比、营收同比、行业、市场、市值等高频支持字段；其余请求再进入 bounded ReAct，由模型选择工具，后端执行本地工具并直接用确定性总结输出最终回答
...
普通自动化测试不依赖真实 AI；`test_agent_query_regression.py` 固化 RC 语料：unsupported metric 必须前置拦截，对话解释/排序/分页/详情不能错误重筛，非确定性 query 仍可进入 fake model / bounded ReAct。PE/PB、ROE、股息率、净利润同比、营收同比、毛利率、负债率、市值、行业、市场、收盘价、换手率，以及低估值、高分红、成长股、稳健、银行股、消费股等支持字段在 Chat SSE 中会先进入 bounded ReAct；只有模型返回通过 schema 校验的工具 action，后端才执行筛选。工具执行成功后不再等待第二次模型总结，而是使用后端确定性总结返回结果，减少真实上游慢导致的等待和 `completion_reason` 噪音。模型慢、超时、不可达或没有给出合法 action 时，Chat Agent 返回普通回复，不自动筛选，并保留 `model_ms` 与 `fallback_reason`。纯问候/缺少条件的澄清请求、名称/代码明确的个股详情请求、unsupported metric 仍保持模型前本地处理。当前 unsupported metric 包括三年 CAGR/复合增速、扣非净利润、经营现金流、EPS/每股收益、PS/市销率、机构持仓、基金持仓、北向资金、研报评级、目标价；命中这些字段时应在 AI health / Qwen / screener 之前停止筛选并解释原因。完整字段边界见 [`FIELD_CAPABILITIES.md`](FIELD_CAPABILITIES.md)。真实 Qwen smoke 只验证运行态：AI 未配置或不健康时 WARN/跳过；Qwen 慢或不可达时安全停止是预期降级。`smoke:chat` 验证 `/chat` 多轮 UI、工具轨迹、结果预览、详情跳转、返回和刷新恢复；`smoke:dashboard` 验证 `/health/data` 的最新/应至交易日、覆盖率、同步任务和交易日说明在浏览器可见；`smoke:detail` 验证 `/detail/600036.SH` 本地详情、K 线和移动端溢出。
