...
自然语言筛选 **SSE 流式版本**。Chat Agent 是 bounded ReAct：除 unsupported metric 字段能力边界会在模型前硬拦截外，意图判断由模型完成。普通问候和帮助类问题应由模型直接 final，不自动调用筛选或策略工具；筛选、内置策略、解释上一轮、排序、分页、确认执行、调严/放宽上一轮条件和个股详情都必须由模型选择一个白名单工具 action。只有模型返回通过 schema 校验的工具 action，后端才执行本地工具并生成 observation。模型慢、超时、上游不可达或没有给出合法 action/final 时，后端返回普通回复，不自动筛选，并保留计时和原因。
