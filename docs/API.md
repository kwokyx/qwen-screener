...
自然语言筛选 **SSE 流式版本**。Chat Agent 是 bounded ReAct：明确指标阈值、行业、板块等高置信支持字段筛选会先由本地确定性解析并执行工具；解释上一轮、排序、分页、确认执行和调严/放宽上一轮条件等上下文操作也会在模型前本地处理。强势突破、均线放量、涨停后承接、高位旗形等明确命中内置策略的表达会本地路由到 `strategy_select`；非白名单策略请求仍需模型选择或安全停止，不能宽泛兜底。普通帮助类对话只返回说明，不自动调用筛选或策略工具。其他需要模型理解、工具路由或策略判断的请求才让模型选择一个白名单工具。只有模型返回通过 schema 校验的工具 action，后端才执行本地工具并生成 observation。纯问候/缺少条件的澄清请求、明确“先别执行”的策略设计请求，以及 unsupported metric 会在模型前本地处理；模型慢、超时、上游不可达或没有给出合法 action 时，后端返回普通回复，不自动筛选，并保留计时和原因。
