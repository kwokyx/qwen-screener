...
本项目是一个面向 A 股场景的智能股票筛选系统，围绕“数据采集与同步、行情与财务数据管理、条件筛选、策略选股、AI 自然语言选股、自选股预警和个股详情分析”构建完整应用。系统后端以本地行情和财务数据库为核心，AI 负责理解用户意图并选择白名单工具，实际筛选、排序、策略计算和数据校验均由服务端确定性执行，避免把模型生成内容直接作为筛选结果。
...
| AI 选股 | `/chat` | 基于千问能力的自然语言交互，支持筛选、解释、排序、分页、详情和策略工具 |
...
AI 选股采用 bounded ReAct 架构：模型只负责理解自然语言目标并输出动作，后端只执行白名单工具，并对参数做 schema 校验。模型超时、不可达、返回非法工具或请求不支持字段时，系统会普通回复或安全停止，不会把宽泛兜底结果伪装成 AI 筛选结果。
